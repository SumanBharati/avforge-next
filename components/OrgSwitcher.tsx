  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-1">
        <span className="text-[9px] font-semibold uppercase tracking-widest text-subtle">Account</span>
    ...
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen((current) => !current)} className="flex min-w-[170px] items-center justify-between gap-3 rounded-lg px-2 py-1 text-left transition-colors hover:bg-forge-surface" aria-haspopup="listbox" aria-expanded={open}>
        <span className="min-w-0">
          <span className="block text-[9px] font-semibold uppercase tracking-widest text-subtle">Account</span>
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-2 w-72 overflow-hidden rounded-xl border border-border bg-forge-bg shadow-2xl">
          <div className="border-b border-border px-3 py-2 text-[10px] font-semibold uppercase tracking-wider text-subtle">Switch Account</div>
          ...
          <div className="grid grid-cols-2 border-t border-border text-xs">
            ...
            <Link href="/org/new" onClick={() => setOpen(false)} className="border-l border-border px-3 py-3 text-center font-medium text-violet-400 transition-colors hover:bg-violet-500/10">New team</Link>
          </div>
        </div>
      )}
    </div>
